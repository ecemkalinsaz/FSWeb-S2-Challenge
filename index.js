...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
	const newArr = cumlelereDonustur(cumleler, " ");
	let arr = []
	...
	return cumleKur(arr[0], arr[1], arr[2], arr[3], arr[4]);
}
...
function emojileriDonustur(mesaj, emojiler) {
	for (let key in emojiler) { //buradaki (var key in emojiler) ifadesi, `emojiler` object'inin her bir key'ini döndürmek için kullanılan bir döngü ifadesidir.
		let sembol = emojiler[key];
		mesaj = mesaj.replaceAll(key.toLowerCase(), sembol)
			.replaceAll(key.toUpperCase(), sembol);
	}
	return mesaj;
}
